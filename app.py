...
if uploaded_post_file and uploaded_ext_file:
    with st.spinner("各时段独立空间指标解耦计算中..."):
        try:
            ...
            if df_post_res is not None:
                ...
                # 指标 1：总来电量（上线后）
                total_calls = int(len(df_rooms_post))
                ...
                with col_kpi1:
                    st.metric(label="📞 总来电量 (上线后时段)", value=f"{total_calls} 次")
                ...
                with col_kpi2:
                    if is_ai_abnormal:
                        ...
                        st.metric(label="🤖 AI接通率 (上线后)", value=ai_rate_str)
                        
                with col_kpi3:
                    st.metric(label="🎧 人工接通率 (上线后新口径)", value=online_man_rate_str)
                    
                with col_kpi4:
                    st.metric(
                        label="👥 整体接通率 (上线前历史人工)", 
                        value=pre_man_rate_str,
                        help="依据您上传的【上线前-历史通话详单】独立空间算出的历史对标值"
                    )
                ...
                df_summary = pd.DataFrame({
                    "指标名称": ["总来电量(上线后)", "AI接通率(上线后)", "人工接通率(上线后)", "整体接通率(上线前历史)"],
                    "计算结果": [total_calls, ai_rate_str, online_man_rate_str, pre_man_rate_str],
                    "时段空间归属与口径说明": [
                        "上线后时段客房呼出总量", 
                        f"上线后时段 {'(⚠️数据异常)' if is_ai_abnormal else '（正常健康）'}", 
                        "上线后时段精细加总口径", 
                        "通过历史时段底表计算，用于对标切换效果"
                    ]
                })
                ...
                with pd.ExcelWriter(excel_buffer, engine='openpyxl') as writer:
                    df_summary.to_excel(writer, sheet_name="时段对比运营看板", index=False)
                    df_post_res.to_excel(writer, sheet_name="上线后清洗详单", index=False)
                    if uploaded_pre_file and 'df_pre_res' in locals():
                        df_pre_res.to_excel(writer, sheet_name="上线前历史清洗详单", index=False)
                        
                st.download_button(
                    label="📥 下载精简版多时段对比报告",
                    data=excel_buffer.getvalue(),
                    file_name="语音双时段复盘运营简报.xlsx",
    ...
    st.info("💡 提示：请上传分机号参考表和上线后的详单底表。若需要对比历史数据，请在空间 B 槽位上传上线前数据。")
